 ...
 * 
 * @file set_symbols.cc
 * @brief Donde se desarrollan las funciones de la clase SetSymbols contenida
 * en el archivo de cabecera set_symbols.h.
 *
...
SetSymbols::SetSymbols(const std::string& name, 
                       const std::set<Symbol>& set_symbols)
...
void SetSymbols::Insert(const Symbol& symbol) {
  set_symbols_.insert(symbol);
}
...
Symbol SetSymbols::At(const size_t& position) const {
  Symbol result("NULL");
  if (position < set_symbols_.size()) {
    size_t count{0};
    for (std::set<Symbol>::iterator i{set_symbols_.begin()}; 
...
std::set<Symbol> SetSymbols::GetSetSymbols(void) const {
  return set_symbols_;
}
...
bool SetSymbols::IsItInSetSymbols(const Symbol& symbol) const {
  bool result{false};
  for (std::set<Symbol>::iterator i{set_symbols_.begin()}; 
       i != set_symbols_.end(); ++i) {
    if (symbol == *i) {
...
/// posición indicada.
Symbol SetSymbols::operator[](const size_t position) const {
...
std::ostream& operator<<(std::ostream& out, const SetSymbols& set_symbols) {
  ...
  if (set_symbols.set_symbols_.size() > 0) {
    for (std::set<Symbol>::iterator i{set_symbols.set_symbols_.begin()}; 
